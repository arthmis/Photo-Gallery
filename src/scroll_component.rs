...
impl ScrollComponent {
    ...
    pub fn calc_horizontal_bar_bounds(
        ...
        let left_x_offset =
            ((viewport_size.width - length - horizontal_padding)
                * percent_scrolled)
                .ceil();
    ...
    pub fn event(
        ...
        if self.are_bars_held() {
            ...
            match event {
                Event::MouseMove(event) => {
                    ...
                    if self.point_hits_vertical_bar(port, offset_pos, env) {
                        ...
                        .point_hits_horizontal_bar(port, offset_pos, env)
                    {
            ...
            match event {
                Event::MouseMove(_) => {
                    // if we have just stopped hovering over scrollbar
                    // set scrollbar hover state to none
                    if self.hovered.is_hovered() && !scrollbar_is_hovered {
                        self.hovered = BarHoveredState::None;
                    // continue displaying scrollbar while scroll component is hovered
                    } else if ctx.is_hot() {
                        self.opacity = env.get(theme::SCROLLBAR_MAX_OPACITY);
                        self.timer_id = TimerToken::INVALID; // Cancel any fade out in progress
                        ctx.request_paint();
