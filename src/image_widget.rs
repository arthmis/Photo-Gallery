...
use druid::{piet::ImageFormat, FileInfo, ImageBuf, Selector};
...
// impl<T: ToImageBuf> Widget<T> for Image {
impl<T: Data> Widget<T> for Image {
    ...
    fn lifecycle(
        ...
        match event {
            LifeCycle::WidgetAdded => {
                dbg!("child image of Thumbnail added");
                // dbg!(data);
                ctx.request_layout();
                ctx.request_paint();
    ...
    fn paint(&mut self, ctx: &mut PaintCtx, _data: &T, _env: &Env) {
        let offset_matrix =
            self.fill.affine_to_fill(ctx.size(), self.image_data.size());
        ...
        ctx.with_save(|ctx| {
            let piet_image = {
                ...
                self.paint_data.get_or_insert_with(|| {
                    image_data.to_piet_image(ctx.render_ctx)
                })
            };
...
pub trait ToImageBuf {
    fn to_image_buf(
        ...
        ImageBuf::from_raw(raw_data, format, width, height)
    }
}
...
//                 .concat();
//                 assert_eq!(
//                     raw_pixels[199 * 600 * 4..200 * 600 * 4],
//                     expecting[..]
//                 );
...
//                 .concat();
//                 assert_eq!(
//                     raw_pixels[399 * 600 * 4..400 * 600 * 4],
//                     expecting[..]
//                 );
//             },
//         );
...
//         let image_widget =
//             Scroll::new(Container::new(Image::new(image_data)).with_id(id_1))
//                 .vertical();
